...
class ResNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        if categories is not None:
            ...
            print(f"Expected total categories: {sum(categories)}")
            print(f"Category Embeddings Size: {self.category_embeddings.num_embeddings}")
        ...
        self.attention = nn.MultiheadAttention(embed_dim=d_in, num_heads=n_heads)
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        x = x.unsqueeze(0)  # (batch_size, seq_len, embed_dim) -> (1, batch_size, embed_dim)
        attn_output, _ = self.attention(x, x, x)  # Self-attention
        x = attn_output.squeeze(0)  # (1, batch_size, embed_dim) -> (batch_size, embed_dim)
